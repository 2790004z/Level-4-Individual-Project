...
class SkateFormer(nn.Module):
    def __init__(self, in_channels=6, depths=(2, 2, 2, 2), channels=(96, 192, 192, 192), num_classes=5,
                 embed_dim=64, num_people=1, num_frames=100, num_points=5, kernel_size=7, num_heads=32,
                 type_1_size=(1, 1), type_2_size=(1, 1), type_3_size=(1, 1), type_4_size=(1, 1),
                 attn_drop=0.1, head_drop=0.1, drop=0.1, rel=True, drop_path=0.1, mlp_ratio=4.,
    ...
    @torch.no_grad()
    def extract_feature(self, x: torch.Tensor, index_t: Optional[torch.Tensor] = None):
        """
        Extracts a spatiotemporal feature map and corresponding per-position logits.

        Args:
            x (Tensor): Input of shape (N, C, T, V, M).
            index_t (Optional[Tensor]): If `self.index_t` is True, pass the index
                values for each frame to compute time embeddings.

        Returns:
            clip_logits (Tensor): (N, num_classes, T', V', M) spatiotemporal map of class scores.
            feats       (Tensor): (N, C_out, T', V', M) feature map after the final stage.
        """
        # 1) Reorder input just like in forward(): (N, C, T, V, M) -> (N, C, T, M*V)
        N, C, T, V, M = x.shape
        x_reordered = x.permute(0, 1, 2, 4, 3).contiguous().view(N, C, T, -1)  # (N, C, T, M*V)

        # 2) Stem layers
        for layer in self.stem:
            x_reordered = layer(x_reordered)

        # 3) Add positional / joint-person embedding
        if self.index_t:
            # sinusoidal temporal embedding for index_t
            te = torch.zeros(N, T, self.embed_dim, device=x_reordered.device)  # (N, T, C)
            div_term = torch.exp(
                -math.log(10000.0) / self.embed_dim *
                torch.arange(0, self.embed_dim, 2, device=x_reordered.device)
            )
            te[:, :, 0::2] = torch.sin(index_t.unsqueeze(-1).float() * div_term)
            te[:, :, 1::2] = torch.cos(index_t.unsqueeze(-1).float() * div_term)
            # Add joint-person embedding
            x_reordered = x_reordered + torch.einsum('n t c, c v -> n c t v',
                                                     te, self.joint_person_embedding)
        else:
            # Use the learnable joint_person_temporal_embedding
            x_reordered = x_reordered + self.joint_person_temporal_embedding

        # 4) Pass through all stages => final feature maps
        feats = x_reordered
        for stage in self.stages:
            feats = stage(feats)
        # feats now shape: (N, c_out, T', V')

        # 5) Build spatiotemporal logits via a 1x1 conv over (T', V')
        #    This parallels how ST-GCN does 'self.fcn(...)' to produce per-position logits
        c_out = feats.size(1)
        logits_conv = nn.Conv2d(c_out, self.num_classes, kernel_size=1).to(feats.device)
        nn.init.normal_(logits_conv.weight, 0, math.sqrt(2.0 / self.num_classes))
        nn.init.constant_(logits_conv.bias, 0)

        clip_logits = logits_conv(feats)  # => (N, num_classes, T', V')
        clip_logits = F.softmax(clip_logits, dim=1)

        # 6) Unsqueeze a dummy M dimension (so shape matches ST-GCN style)
        clip_logits = clip_logits.unsqueeze(-1)  # (N, num_classes, T', V', M=1)
        feats = feats.unsqueeze(-1)              # (N, c_out,     T', V', M=1)

        return clip_logits, feats


def SkateFormer_(dropout=0.1, **kwargs):
    return SkateFormer(
        depths=(2, 2, 2, 2),
